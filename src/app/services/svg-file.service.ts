...
import { Observable as ObserverType } from "../types/observer";


export interface SavageSVG extends Omit<INode, "attributes" | "children"> {
	nid: string;
	attributes: ObserverType<Record<string, string>>;
	children: ObserverType<ObserverType<SavageSVG>[]>;
}
...
export function recursiveUnobserve(tree: ObserverType<SavageSVG>): void {
	tree.unobserve();
